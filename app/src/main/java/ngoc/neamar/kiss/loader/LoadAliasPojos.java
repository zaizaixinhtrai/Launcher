...
import ngoc.neamar.kiss.pojo.AliasPojo;
...
public class LoadAliasPojos extends LoadPojos<AliasPojo> {
    ...
    protected ArrayList<AliasPojo> doInBackground(Void... params) {
        ...
        if (phoneApp != null) {
            String phoneAlias = context.getResources().getString(ngoc.neamar.kiss.R.string.alias_phone);
        ...
        if (android.os.Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH_MR1) {
            ...
            if (contactApp != null) {
                String contactAlias = context.getResources().getString(ngoc.neamar.kiss.R.string.alias_contacts);
                addAliasesPojo(alias, contactAlias.split(","), contactApp);
            }

            String browserApp = getAppByCategory(pm, Intent.CATEGORY_APP_BROWSER);
            if (browserApp != null) {
                String webAlias = context.getResources().getString(ngoc.neamar.kiss.R.string.alias_web);
                addAliasesPojo(alias, webAlias.split(","), browserApp);
            }

            String mailApp = getAppByCategory(pm, Intent.CATEGORY_APP_EMAIL);
            if (mailApp != null) {
                String mailAlias = context.getResources().getString(ngoc.neamar.kiss.R.string.alias_mail);
                addAliasesPojo(alias, mailAlias.split(","), mailApp);
            }

            String marketApp = getAppByCategory(pm, Intent.CATEGORY_APP_MARKET);
            if (marketApp != null) {
                String marketAlias = context.getResources().getString(ngoc.neamar.kiss.R.string.alias_market);
            ...
            if (messagingApp != null) {
                String messagingAlias = context.getResources().getString(ngoc.neamar.kiss.R.string.alias_messaging);
                addAliasesPojo(alias, messagingAlias.split(","), messagingApp);
            }

            String clockApp = getClockApp(pm);
            if (clockApp != null) {
                String clockAlias = context.getResources().getString(ngoc.neamar.kiss.R.string.alias_clock);

...
public class PhoneResult extends Result {
    ...
    public View display(Context context, int position, View v) {
        if (v == null)
            v = inflateFromId(context, ngoc.neamar.kiss.R.layout.item_phone);

        TextView appName = (TextView) v.findViewById(ngoc.neamar.kiss.R.id.item_phone_text);
        String text = context.getString(ngoc.neamar.kiss.R.string.ui_item_phone);
        appName.setText(enrichText(String.format(text, "{" + phonePojo.phone + "}")));

        ((ImageView) v.findViewById(ngoc.neamar.kiss.R.id.item_phone_icon)).setColorFilter(getThemeFillColor(context), PorterDuff.Mode.SRC_IN);
    ...
    protected PopupMenu buildPopupMenu(Context context, final RecordAdapter parent, View parentView) {
        return inflatePopupMenu(ngoc.neamar.kiss.R.menu.menu_item_phone, context, parentView);
    }

    @Override
    protected Boolean popupMenuClickHandler(Context context, RecordAdapter parent, MenuItem item) {
        switch (item.getItemId()) {
            case ngoc.neamar.kiss.R.id.item_phone_createcontact:
                ...
                return true;
            case ngoc.neamar.kiss.R.id.item_phone_sendmessage:

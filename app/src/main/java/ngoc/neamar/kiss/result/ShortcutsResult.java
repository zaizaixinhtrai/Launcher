...
public class ShortcutsResult extends Result {
    ...
    public View display(final Context context, int position, View v) {
        if (v == null)
            v = inflateFromId(context, ngoc.neamar.kiss.R.layout.item_shortcut);

        TextView appName = (TextView) v.findViewById(ngoc.neamar.kiss.R.id.item_app_name);
        appName.setText(enrichText(shortcutPojo.displayName));

        final ImageView shortcutIcon = (ImageView) v.findViewById(ngoc.neamar.kiss.R.id.item_shortcut_icon);
        final ImageView appIcon = (ImageView) v.findViewById(ngoc.neamar.kiss.R.id.item_app_icon);
    ...
    protected void doLaunch(Context context, View v) {
        ...
        try {
            ...
            Toast.makeText(context, ngoc.neamar.kiss.R.string.application_not_found, Toast.LENGTH_LONG).show();
        }
    ...
    PopupMenu buildPopupMenu(Context context, RecordAdapter parent, View parentView) {
        return inflatePopupMenu(ngoc.neamar.kiss.R.menu.menu_item_shortcut, context, parentView);
    }

    @Override
    Boolean popupMenuClickHandler(Context context, RecordAdapter parent, MenuItem item) {
        switch (item.getItemId()) {
            case ngoc.neamar.kiss.R.id.item_app_uninstall:
